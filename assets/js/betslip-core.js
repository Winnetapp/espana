/* =============================================================
   ...
   Responsabilidades:
   · Almacén de apuestas (localStorage)
   · Exclusión de mercados incompatibles (reemplaza automáticamente)
   · Cálculo de cuota con correlaciones (mismo partido)
   · Apuestas de sistema (2/3, 3/4, etc.)
   · Render del bet slip (desktop + móvil)
   · Toast de notificaciones
   · Expone window.BetSlip para que main.js y partido.js lo usen

   ANTI-TRAMPAS:
   · addBet() rechaza partidos con estado en vivo (1H/HT/2H/ET/P)
   · realizarApuesta() verifica en Firestore el estado actual de
     CADA partido del carrito antes de guardar — si alguno está
     en vivo cancela la apuesta completa aunque fuera añadida antes
   ============================================================= */
...
(function () {
  ...
  /* ─────────────────────────────────────────────────────────
     ANTI-TRAMPAS — VERIFICACIÓN EN VIVO
     Comprueba en Firestore si un partido está en curso.
     Busca primero en 'partidos', luego en 'historial'.
     Devuelve true si está en vivo (debe bloquearse).
  ───────────────────────────────────────────────────────── */
  async function partidoEnVivo(partidoId) {
    ...
    try {
      ...
      if (snap.exists) {
        return ESTADOS_VIVO.includes(snap.data()?.estado);
      }
      // por si acaso está en historial (no debería, pero cubre el caso)
  ...
  function factorCorrelacion(a, b) {
    ...
    if (esGoles(mA) && esGoles(mB)) {
      ...
      if (dA === dB && dA === 'mas' && nA !== null && nB !== null) {
        if (nA >= nB || nB >= nA) return 0.7;
      }
    ...
    if (esCorn(mA) && esCorn(mB)) {
      ...
      if (dA === dB && nA !== null && nB !== null) {
        if (dA === 'mas'   && (segA !== 'encuentro') && segB === 'encuentro' && nA >= nB) return 0;
        if (dA === 'menos' && segA === 'encuentro' && (segB !== 'encuentro') && nA <= nB) return 0;
      }
      return 0.5;
    }
  ...
  /* ─────────────────────────────────────────────────────────
     CÁLCULO DE CUOTA COMBINADA CON CORRELACIONES
  ───────────────────────────────────────────────────────── */
  function calcularCuotaCombinada(bets) {
    ...
    for (const grupo of Object.values(porPartido)) {
      ...
      for (let i = 1; i < activas.length; i++) {
        ...
        if (factorMin > 0 && factorMin < 1) {
          cuotaPartido *= 1 + (cuotaNueva - 1) * factorMin;
        } else if (factorMin === 1) {
          cuotaPartido *= cuotaNueva;
        }
      }
      total *= cuotaPartido;
    }
  ...
  /* ─────────────────────────────────────────────────────────
     AÑADIR APUESTA — con verificación anti-trampas
  ───────────────────────────────────────────────────────── */
  async function addBet({ partido, tipo, cuota, partidoId, mercado }) {
    ...
    /* ── ANTI-TRAMPAS: verificar que el partido no está en vivo ── */
    if (pid) {
      const enVivo = await partidoEnVivo(pid);
      if (enVivo) {
        toast('⚠ No se puede apostar en partidos en vivo', 'error');
        return;
      }
    }
  ...
  function renderListas() {
    const html = bets.length ? buildListaHTML() : `
      <li class="bs-empty">
        <span>🎯</span>
        <span>Selecciona una cuota para apostar</span>
      </li>`;

    document.querySelectorAll('.bs-list, #slip-list, #slip-list-modal').forEach(el => {
      el.innerHTML = html;
      el.querySelectorAll('.bs-remove').forEach(btn =>
        btn.addEventListener('click', e => {
          e.stopPropagation();
          const idx  = parseInt(btn.dataset.idx, 10);
          const item = btn.closest('.bs-item, .slip-item, li, div[class*="bs-item"]');
          if (item) {
  ...
  function buildListaHTML() {
    ...
    return orden.map(pid => {
      ...
      const titulo     = grupo[0].partido;
      const subItems   = grupo.map(({ tipo, cuota, mercado, _idx }) => {
        const factor = infoCorrelacion(bets, _idx);
        let badge = '';
        if (factor === 0)    badge = `<span class="bs-corr-badge bs-corr-incompatible" title="Cuota absorbida por ser imposible combinada">⚠ Absorbida</span>`;
        else if (factor < 1) badge = `<span class="bs-corr-badge bs-corr-reducida" title="Cuota reducida por correlación entre mercados">↘ Correlada</span>`;
        return `
      ...
      return `
        <li class="bs-item bs-item-grupo-header">
          <div class="bs-top">
            <span class="bs-tipo bs-grupo-nombre">⚽ ${titulo}</span>
  ...
  function renderSistema() {
    ...
    const selecciones = Object.values(porPartido);
    ...
    let html = `<div class="sistema-info">Sistema de ${n} selecciones</div>`;
    html += `<div class="sistema-opciones">`;

    for (let k = 2; k < n; k++) {
      const { combos, cuotaMedia, retornoTotal } = calcularSistema(selecciones, k);
      html += `
        <label class="sistema-opcion" for="sis-${k}">
    ...
    const stakeEl = document.querySelector('.stake-input input');
    const stakeVal = parseFloat(stakeEl?.value || 5) || 5;
    const kSel = parseInt(wrap.querySelector('input[name="sistema-tipo"]:checked')?.value || 2);
    const { combos: combosActual } = calcularSistema(selecciones, kSel);
    const totalApostado = stakeVal * combosActual;

    html += `
      <div class="sistema-footer">
        <div class="sis-detalle-apuesta">
          <span class="sis-label">Importe por combinación</span>
          <span class="sis-valor">${stakeVal.toFixed(2)} €</span>
        </div>
        <div class="sis-detalle-apuesta">
          <span class="sis-label">Total apostado</span>
          <span class="sis-valor">${totalApostado.toFixed(2)} €</span>
        </div>
      </div>`;
  ...
  function renderFooter() {
    ...
    const stakeEl = document.querySelector('.stake-input input, #slip-stake, #slip-stake-modal');
  ...
  function actualizarBotones() {
    document.querySelectorAll('.cuota-btn').forEach(btn => {
      const pid     = btn.dataset.partidoid;
      const tipo    = btn.dataset.tipo;
      const mercado = btn.dataset.mercado || 'resultado';
      const activa  = bets.some(b =>
        (b.partidoId || '').toString() === pid &&
        (b.tipo   || '').toLowerCase() === (tipo    || '').toLowerCase() &&
        normM(b.mercado) === normM(mercado)
      );
  ...
  /* ─────────────────────────────────────────────────────────
     ...
     Segunda capa anti-trampas: verifica en tiempo real el
     estado de CADA partido antes de confirmar la apuesta.
  ───────────────────────────────────────────────────────── */
  async function realizarApuesta() {
    ...
    /* ── ANTI-TRAMPAS CAPA 2: verificar estado actual de cada partido ── */
    ...
    if (pidsBloqueados.length) {
      /* Eliminar del carrito las apuestas de partidos en vivo */
      bets = bets.filter(b => {
        const pid = (b.partidoId || '').toString().trim();
        return !idsUnicos.some(id => id === pid && pidsBloqueados.some(n => bets.find(x => x.partidoId === pid)?.partido === n || id === pid));
      });
      /* Forma más directa: construir el set de pids bloqueados */
      const pidsBloq = new Set(
        bets
          .filter(b => pidsBloqueados.includes(b.partido))
          .map(b => b.partidoId)
      );
      // Limpiar todos los que coincidan con los partidos detectados en vivo
    ...
    const stakeEl   = document.querySelector('.stake-input input, #slip-stake');
    const stake     = parseFloat(stakeEl?.value || 5) || 0;
    ...
    if (tabActual === 'sistema' && bets.length >= 3) {
      ...
      const kEl          = document.querySelector('input[name="sistema-tipo"]:checked');
      const k            = parseInt(kEl?.value || 2);
      ...
      const cuotaTotal = calcularCuotaCombinada(sels);
      apuestaData = {
        usuarioId: user.uid,
        ...
        totalOdds:    cuotaTotal,
        potentialWin: stake * cuotaTotal * combos,
  ...
  function init() {
    ...
    document.querySelectorAll('.stake-input input, #slip-stake, #slip-stake-modal').forEach(el => {
      el.addEventListener('input', () => {
        const val = el.value;
        document.querySelectorAll('#slip-stake, #slip-stake-modal').forEach(o => {
          if (o !== el) o.value = val;
        });
        renderFooter();
      });
    ...
    document.querySelectorAll('#clear-bets, .slip-clear-btn, #slip-clear-btn, #slip-clear-btn-modal').forEach(el =>
      el?.addEventListener('click', () => { vaciar(); toast('Carrito vaciado', 'info'); })
    );

    document.querySelectorAll('.accept-btn, #btn-apostar, #btn-apostar-modal').forEach(el =>
      el?.addEventListener('click', realizarApuesta)
    );

    /* Sidebar móvil (index.html) */
    const mobileBtn    = document.getElementById('mobileMenuBtn');
    const sidebarEl    = document.getElementById('userSidebar');
    const overlayIndex = document.getElementById('sidebar-overlay');
    const bsToggle     = document.querySelector('.bs-toggle');

    const openSidebar  = () => { sidebarEl?.classList.add('open'); overlayIndex && (overlayIndex.style.display = 'block', overlayIndex.classList.add('active')); document.body.style.overflow = 'hidden'; };
    const closeSidebar = () => { sidebarEl?.classList.remove('open'); overlayIndex && (overlayIndex.style.display = 'none', overlayIndex.classList.remove('active')); document.body.style.overflow = ''; };

    mobileBtn?.addEventListener('click', openSidebar);
    overlayIndex?.addEventListener('click', closeSidebar);
    bsToggle?.addEventListener('click', closeSidebar);

    /* Modal móvil (partido.html) */
  ...
  window.addBet       = (tipo, cuota, mercado) => {
    ...
    addBet({
      ...
      tipo, cuota: String(cuota), mercado,
