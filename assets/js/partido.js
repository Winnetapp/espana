...
function renderizarPartido(partido) {
  ...
  let nacionalidad1 = partido.nacionalidad1 || "";
  let nacionalidad2 = partido.nacionalidad2 || "";
  let bandera1 = nacionalidad1
    ? `banderas/${removeTildes(nacionalidad1.toLowerCase().replace(/\s/g, ''))}.png`
    : "";
  let bandera2 = nacionalidad2
    ? `banderas/${removeTildes(nacionalidad2.toLowerCase().replace(/\s/g, ''))}.png`
    : "";
  ...
  // --- CAMBIO PRINCIPAL: BANDERAS PARA TENIS ---
  let equipo1HTML, equipo2HTML;
  if (deporte.toLowerCase() === "tenis") {
    equipo1HTML = `
      <div class="bandera-wrapper">
        ${bandera1 ? `<img src="${bandera1}" class="bandera" alt="${nacionalidad1 || partido.equipo1}" />` : ""}
      </div>
    `;
    equipo2HTML = `
      <div class="bandera-wrapper">
        ${bandera2 ? `<img src="${bandera2}" class="bandera" alt="${nacionalidad2 || partido.equipo2}" />` : ""}
      </div>
    `;
  } else {
    equipo1HTML = `<div class="escudo-wrapper"><img src="${escudo1}" class="escudo" alt="${partido.equipo1}" /></div>`;
    equipo2HTML = `<div class="escudo-wrapper"><img src="${escudo2}" class="escudo" alt="${partido.equipo2}" /></div>`;
  }
    ...
    <div class="partido-header">
      ...
      <div class="partido-info">
        ...
        <div class="partido-datos">
          ...
          <span class="partido-hora">${horaFormateada}</span>
        </div>
      </div>
      ${equipo2HTML}
    </div>
...
// ====== NUEVOS MERCADOS DESPLEGABLES ======
function renderizarMercados(mercados, partido) {
  const mercadosDiv = document.getElementById('mercados-partido');
  mercadosDiv.innerHTML = '';

  // ... (mercados anteriores: resultado, goleadores, tarjetas, corners, etc.)
  ...
  // ... (desplegables de tarjetas y corners avanzado, y demás mercados)
